...
*Current status: Phase 0 (web presence), the Phase 1 data spine, and Phase 2 AI enrichment are built. Phase 1: user auth (email + Apple Sign-In verification, rotating refresh tokens), the provider abstraction (`apps/api/src/providers/`, Plaid + mock), encrypted token storage, webhook-driven cursor sync with dedupe/pending-reconciliation/transfer-pair detection, delete/disconnect flows, and the iOS link harness (`apps/ios`). Phase 2: the enrichment provider abstraction (`apps/api/src/enrichment/`, real Haiku via structured outputs + a deterministic mock/fallback), batched categorization wired into the sync pipeline, rule-based recurring-stream detection, nightly per-user feature-store rollups, the user-correction loop with per-user few-shot injection, AI cost metering, and a 500-transaction eval harness gating ≥90% discretionary/essential accuracy on a 400-row held-out set (currently 92.5% against the mock provider). Phase 1's exit gate (link a real account from the harness) and Phase 2's (≥90% accuracy against live Haiku, cost/user/month measured) both still need real credentials (Plaid sandbox keys, `ANTHROPIC_API_KEY`) to run end-to-end outside this environment — the mock-provider paths are fully tested and pass. Next: run both exit gates with real credentials, then Phase 3 (coaching engine). The coaching-policy prompt remains the artifact worth the most care.*
