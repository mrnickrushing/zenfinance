...
*Current status: Phases 0–3 are built. Phase 0 (web presence), Phase 1 (data spine: auth + Apple Sign-In, provider abstraction, encrypted tokens, webhook cursor sync with dedupe/reconciliation/transfer-pairs, delete/disconnect, iOS harness), and Phase 2 (AI enrichment: real Haiku via structured outputs + deterministic mock/fallback, batched categorization, recurring-stream detection, nightly feature-store rollups, user-correction few-shot loop, cost metering; 500-transaction eval at 92.5% held-out discretionary accuracy). Phase 3 (coaching engine, `apps/api/src/coaching/`): the insight provider abstraction (real Sonnet brief via structured outputs + a deterministic mock + a template fallback), deterministic derived-figure assembly (goal pacing, category deltas, citable facts with stable aggregate ids — the model never does arithmetic), the coaching-policy prompt, and the §4 guard chain (semantic provenance check binding every cited figure to a ground-truth fact, banned-phrase tone pass, retry-once-then-template fallback); goals CRUD with pacing/projection; rule-based anomaly detection (duplicate/unusual/fee/new-recurring); subscription auditor v1 (monthly-normalized, price-creep flags, cancellation scripts); the Money Wins ledger with the estimated-vs-verified split and automated charge-absence verification (2 clean cycles) plus user confirmation; the first-look brief generated at link time and a weekly-brief job; and a 50-persona eval suite that gates every brief on dollar-figure + action + provenance + tone (50/50 passing against the mock coach and the template fallback). Exit gates for Phases 1–3 (link a real account; ≥90% enrichment accuracy against live Haiku; a weekly brief on real data worth screenshotting) need real credentials (Plaid sandbox, `ANTHROPIC_API_KEY`) to run end-to-end outside this environment — the mock-provider paths are fully tested and pass. Next: run the exit gates with real credentials, then Phase 4 (the iOS app).*
